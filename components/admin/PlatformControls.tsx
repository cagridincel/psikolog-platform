  ...
  return (
    <div className="space-y-3 max-w-2xl">
      {toggleSettings.map(s => {
        ...
        return (
          <div key={s.key} className="bg-white rounded-2xl border p-5" style={{ borderColor: C.border }}>
            <div className="flex items-start justify-between gap-4">
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1">
                  <p className="text-sm font-medium" style={{ color: C.navy }}>{meta?.label ?? s.key}</p>
                  <span className="text-xs px-2 py-0.5 rounded-full font-medium"
                    style={{ background: isOn ? C.successTint : C.bg, color: isOn ? C.success : C.muted }}>
                    {isOn ? 'Aktif' : 'Pasif'}
                  </span>
                </div>
                <p className="text-sm leading-relaxed" style={{ color: C.muted }}>{meta?.detail ?? s.description}</p>
                <p className="text-xs mt-2" style={{ color: C.muted }}>Son güncelleme: {formatDate(s.updated_at)}</p>
              </div>
              <button onClick={() => saveValue(s.key, !isOn)} disabled={saving === s.key}
                className="flex-shrink-0 relative disabled:opacity-60"
                style={{ background: 'none', border: 'none', cursor: 'pointer' }}>
                <div className="w-12 h-6 rounded-full transition-all duration-200"
                  style={{ background: isOn ? C.blue : '#D1D5DB' }}>
            ...
            {s.key === 'single_psychologist_restriction' && isOn && (
      ...
      {numberSettings.map(s => {
        ...
        return (
          <div key={s.key} className="bg-white rounded-2xl border p-5" style={{ borderColor: C.border }}>
            <div className="flex items-start justify-between gap-4">
              <div className="flex-1 min-w-0">
                <p className="text-sm font-medium mb-1" style={{ color: C.navy }}>{meta?.label ?? s.key}</p>
                <p className="text-sm leading-relaxed" style={{ color: C.muted }}>{meta?.detail ?? s.description}</p>
                <p className="text-xs mt-2" style={{ color: C.muted }}>Son güncelleme: {formatDate(s.updated_at)}</p>
              </div>
              <div className="flex items-center gap-2 flex-shrink-0">
                <input type="number" min={1} max={300} defaultValue={numVal}
                  onBlur={e => { const v = parseInt(e.target.value); if (!isNaN(v)) saveValue(s.key, v) }}
                ...
                {saved === s.key && <span className="text-xs" style={{ color: C.success }}>✓</span>}
              </div>
            </div>
          </div>
        )
