...
export default function PlatformControls() {
  ...
  useEffect(() => {
    fetch('/api/admin/settings')
      .then(r => r.json())
      .then(data => { setSettings((data ?? []).filter((s: any) => !['session_early_join_minutes', 'session_duration_minutes'].includes(s.key))); setLoading(false) })
  }, [])
            ...
            {setting.key === 'single_psychologist_restriction' && setting.value && (
              <div className="mt-4 flex items-start gap-2 px-3 py-2.5 rounded-xl" style={{ background: C.warningTint }}>
                ...
                <p className="text-xs" style={{ color: C.warning }}>
                  Bu kısıtlama aktifken danışanlar yalnızca mevcut psikologlarıyla devam edebilir. Yeni danışanlar istedikleri psikologu seçebilir.
                </p>
              </div>
            )}
          </div>
