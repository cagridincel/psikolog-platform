        ...
        <div className="flex-1 p-8 max-w-3xl">
          ...
          {activeTab === 'home' && (
            <div className="space-y-6">
              {/* Karşılama */}
              <div className="bg-gradient-to-r from-violet-600 to-violet-500 rounded-2xl p-7 text-white relative overflow-hidden">
                ...
                <p className="text-violet-200 text-sm mb-1">Hoş geldin</p>
              ...
              {psychologist ? (
                <div className="bg-white rounded-2xl border border-gray-100 p-6">
                  <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-4">Psikologunuz</p>
                  <div className="flex items-start gap-4">
                    <div className="w-14 h-14 rounded-full bg-violet-100 flex items-center justify-center text-violet-700 text-xl font-bold flex-shrink-0 overflow-hidden">
                      {psychologist.avatar_url
                        ? <img src={psychologist.avatar_url} className="w-14 h-14 object-cover" alt="" />
                        : psychologist.full_name[0]}
                    </div>
                    <div className="flex-1 min-w-0">
                      <h2 className="font-semibold text-gray-900">{psychologist.full_name}</h2>
                      <p className="text-sm text-gray-500 mt-0.5 line-clamp-2">{psychologist.bio}</p>
                    ...
                    <Link
                      ...
                      className="flex-shrink-0 bg-violet-600 text-white text-sm font-medium px-4 py-2 rounded-xl hover:bg-violet-700 transition-colors"
                    >
                      Seans Al
                    </Link>
                  </div>
                  ...
                  <div className="mt-5 pt-5 border-t border-gray-100 flex items-center justify-between">
                    <div>
                      <p className="text-3xl font-bold text-gray-900">{remainingSessions}</p>
                      <p className="text-sm text-gray-400">kalan seans</p>
                    </div>
                ...
                <div className="bg-white rounded-2xl border border-gray-100 p-8 text-center">
                  ...
                  <p className="font-semibold text-gray-800 mb-1">Henüz bir psikolog seçmediniz</p>
                  <p className="text-sm text-gray-400 mb-5">Size uygun psikologu bulmak için değerlendirmeyi deneyin.</p>
                  <Link href="/" className="bg-violet-600 text-white text-sm font-medium px-5 py-2.5 rounded-xl hover:bg-violet-700 transition-colors">
                    Psikolog Bul
                  </Link>
                </div>
              )}
              ...
              <div className="bg-white rounded-2xl border border-gray-100 p-6">
                <div className="flex items-center justify-between mb-4">
                  <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide">Yaklaşan Seanslar</p>
                  {upcomingAppointments.length > 0 && (
                    <button onClick={() => setActiveTab('sessions')} className="text-xs text-violet-600 font-medium hover:underline">
                      Hepsini Gör
                    </button>
                  )}
                </div>
                {upcomingAppointments.length === 0 ? (
                  <p className="text-sm text-gray-400 py-4 text-center">Planlanmış seans bulunmuyor.</p>
                ) : (
          ...
          {activeTab === 'sessions' && (
            <div className="space-y-4">
              <h2 className="text-xl font-bold text-gray-900">Seanslarım</h2>
              {upcomingAppointments.length === 0 ? (
                <div className="bg-white rounded-2xl border border-gray-100 p-12 text-center">
                  <p className="text-gray-400">Planlanmış seans bulunmuyor.</p>
                  {psychologist && (
                    <Link href={`/client/book/${psychologist.id}`} className="mt-4 inline-block bg-violet-600 text-white text-sm font-medium px-5 py-2.5 rounded-xl">
                      Seans Al
                    </Link>
                  )}
                </div>
          ...
          {activeTab === 'psychologist' && (
            <div className="space-y-4">
              <h2 className="text-xl font-bold text-gray-900">Psikologum</h2>
              {psychologist ? (
                <div className="bg-white rounded-2xl border border-gray-100 p-8">
                  <div className="flex items-start gap-5 mb-6">
                    <div className="w-20 h-20 rounded-2xl bg-violet-100 flex items-center justify-center text-violet-700 text-2xl font-bold overflow-hidden flex-shrink-0">
                      {psychologist.avatar_url
                        ? <img src={psychologist.avatar_url} className="w-20 h-20 object-cover" alt="" />
                        : psychologist.full_name[0]}
                    </div>
                    <div>
                      <h3 className="text-xl font-bold text-gray-900">{psychologist.full_name}</h3>
                      <p className="text-gray-500 mt-1">{psychologist.bio}</p>
                  ...
                  <div className="flex gap-3">
                    <Link href={`/client/book/${psychologist.id}`} className="flex-1 text-center bg-violet-600 text-white text-sm font-medium py-3 rounded-xl hover:bg-violet-700 transition-colors">
                      Yeni Seans Al
                    </Link>
                  </div>
                </div>
                ...
                <div className="bg-white rounded-2xl border border-gray-100 p-12 text-center">
                  <p className="text-gray-400 mb-4">Henüz bir psikolog seçmediniz.</p>
                  <Link href="/" className="bg-violet-600 text-white text-sm font-medium px-5 py-2.5 rounded-xl">
                    Psikolog Bul
                  </Link>
                </div>
              )}
              ...
              <div className="space-y-4">
                {notifications.slice(0, 4).map(n => (
                  <div key={n.id} className="flex items-start gap-3">
                    <div className={`w-2 h-2 rounded-full mt-1.5 flex-shrink-0 ${!n.is_read ? 'bg-violet-500' : 'bg-gray-200'}`} />
...
function AppointmentCard({ appointment, expanded = false }: { appointment: Appointment; expanded?: boolean }) {
  ...
  return (
    <div className={`rounded-xl border ${expanded ? 'border-gray-200 p-5' : 'border-gray-100 bg-gray-50 p-4'}`}>
      <div className="flex items-start justify-between gap-3">
        <div>
          {dateStr && (
            <>
              <p className="text-sm font-semibold text-gray-900">{formatDate(dateStr)}</p>
              <p className="text-sm text-gray-500 mt-0.5">{formatTime(dateStr)}</p>
            </>
          )}
          <div className="mt-2">
